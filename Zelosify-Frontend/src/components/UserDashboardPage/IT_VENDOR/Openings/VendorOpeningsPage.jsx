...
import { fetchVendorOpenings } from "@/redux/features/Dashboard/vendorSlice";
...
export default function VendorOpeningsPage() {
  ...
  const { openings, loading, totalOpenings, currentPage, totalPages } = useSelector((s) => s.vendor);
  const [search, setSearch] = useState("");
  const [statusFilter, setStatusFilter] = useState("ALL");
  const [page, setPage] = useState(1);

  useEffect(() => { dispatch(fetchVendorOpenings({ page, limit: 10 })); }, [dispatch, page]);

  const filtered = openings.filter((o) => {
    const q = search.toLowerCase();
    return (
      (o.title.toLowerCase().includes(q) || o.location?.toLowerCase().includes(q) || o.contractType?.toLowerCase().includes(q)) &&
    ...
    <div className="aurora-bg relative min-h-screen p-6 space-y-6">
      ...
      <div className="space-y-1 relative z-10">
        ...
        <p className="text-sm" style={{ color: "var(--aurora-text-muted)" }}>
          Browse positions and submit candidate profiles
        </p>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-4 gap-4 relative z-10">
        <StatCard icon={TrendingUp}  label="Total"   value={totalOpenings} accent="bg-blue-500/20 text-blue-600 dark:text-blue-400"    glow="rgba(79,142,247,0.25)" />
        ...
        <StatCard icon={XCircle}     label="Closed"  value={stats.closed}  accent="bg-red-500/20 text-red-600 dark:text-red-400"         glow="rgba(239,68,68,0.25)" />
      </div>

      {/* Filters */}
      <div className="flex items-center gap-3 flex-wrap relative z-10">
        <div className="relative flex-1 min-w-[220px] max-w-sm">
          <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4" style={{ color: "var(--aurora-text-faint)" }} />
          <input
            type="text"
        ...
        <table className="w-full text-sm">
          <thead>
            <tr style={{ borderBottom: "1px solid var(--aurora-border-subtle)" }}>
              {["Title", "Location", "Type", "Experience", "Posted", "Status", ""].map((h) => (
                <th
                  key={h}
              ...
              <tr>
                <td colSpan={7} className="px-5 py-20 text-center">
                  <div className="w-12 h-12 rounded-2xl flex items-center justify-center mx-auto mb-3"
                    ...
                    <Briefcase className="w-6 h-6" style={{ color: "var(--aurora-text-faint)" }} />
                  </div>
                  <p className="text-sm" style={{ color: "var(--aurora-text-muted)" }}>No openings found</p>
                  {search && (
                    <button onClick={() => setSearch("")}
                      className="text-blue-500 dark:text-blue-400 text-xs mt-2 hover:underline">
                      Clear search
                    </button>
                  )}
                </td>
              ...
              filtered.map((opening) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-5 py-4">
                      <ChevronRight
                        className="w-4 h-4 transition-all group-hover:text-blue-500 dark:group-hover:text-blue-400 group-hover:translate-x-0.5"
                        style={{ color: "var(--aurora-text-faint)" }}
                      />
                    </td>
                  </tr>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between relative z-10">
          <p className="text-xs" style={{ color: "var(--aurora-text-faint)" }}>
            Page {currentPage} of {totalPages} · {totalOpenings} total openings
          </p>
          <div className="flex gap-2">
            {[
              ...
              <button
                ...
                onClick={action}
                disabled={disabled}
                className="px-4 py-2 rounded-xl text-xs transition-all disabled:opacity-30"
                style={{
                  border: "1px solid var(--aurora-border)",
                  background: "var(--aurora-surface)",
